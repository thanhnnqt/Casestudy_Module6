            ...
            <div className="main">
                ...
                <div className="glass-card p-3 mb-4" style={{ position: 'relative', zIndex: 10 }}>
                    <div className="row g-3 mb-3">
                        <div className="col-md-4">
                            ...
                            <input name="keyword" className="form-control custom-input" placeholder="Hãng / Số hiệu..." onChange={handleFilterChange} list="kwOpt"/>
                            <datalist id="kwOpt">{suggestionAirlines.map(a => <option key={a.id} value={a.name}/>)}{suggestionFlightNums.map((n,i) => <option key={i} value={n}/>)}</datalist>
                        </div>
                        <div className="col-md-4">
                            ...
                            <div className="price-range-container">
                                ...
                                <div className="range-slider-wrapper">
                                    <div className="slider-track"><div className="slider-range" style={getProgressBarStyle()}></div></div>
                                    <input type="range" min="0" max="20000000" step="500000" value={priceRange.min} onChange={(e) => handlePriceRangeChange(e, 'min')} className="range-slider-input" />
                                    <input type="range" min="0" max="20000000" step="500000" value={priceRange.max} onChange={(e) => handlePriceRangeChange(e, 'max')} className="range-slider-input" />
                                </div>
                            </div>
                        </div>
                        <div className="col-md-4">
                            ...
                            <div className="date-range-picker-container" ref={datePickerRef}>
                                <div className="custom-input d-flex align-items-center justify-content-between date-input-display" onClick={() => setShowDatePicker(!showDatePicker)}>
                                    <span>{formatDateDisplay()}</span><i className="bi bi-calendar-range"></i>
                                </div>
                                {showDatePicker && (<div className="date-picker-dropdown-calendar"><DateRangePicker ranges={dateRange} onChange={handleDateRangeSelect} months={2} direction="horizontal" locale={vi} showDateDisplay={false} moveRangeOnFirstSelection={false} rangeColors={['#764ba2']} className="custom-date-range-picker" /></div>)}
                            </div>
                        </div>
                    </div>
                ...
                <div className="sort-toolbar shadow-sm d-flex justify-content-between">
                    ...
                    <div className="view-mode-group">
                        <button className={`btn btn-view-mode ${viewMode === 'table' ? 'active' : ''}`} onClick={() => setViewMode('table')} title="Dạng bảng">
                            <i className="bi bi-table"></i>
                        </button>
                        <button className={`btn btn-view-mode ${viewMode === 'card' ? 'active' : ''}`} onClick={() => setViewMode('card')} title="Dạng thẻ">
                ...
                <div className="flight-list-wrapper">
                    <div className="d-flex justify-content-between align-items-center mb-3 px-2">
                        <h6 className="fw-bold mb-0 text-secondary" style={{ fontSize: '0.95rem' }}>Tìm thấy {flights.length} chuyến bay</h6>
