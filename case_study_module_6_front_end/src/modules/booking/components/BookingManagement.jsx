import React, { useEffect, useState } from 'react'; // Bỏ useRef vì không cần nữa
...
const BookingManagement = () => {
    ...
    const fetchBookings = () => {
        ...
        FlightService.getAllBookings()
            ...
            .catch(err => {
                ...
                // Thêm toastId để không hiện lỗi trùng lặp khi tải trang
                toast.error("Không thể tải danh sách!", { toastId: 'fetch-error' });
            })
    ...
    // [FIX QUAN TRỌNG] Sửa lỗi hiện 2 Toast thông báo
    useEffect(() => {
        if (location.state?.updated) {
            // Thêm thuộc tính toastId. Nếu toast có ID này đang hiện, nó sẽ không hiện cái mới.
            toast.success("Cập nhật vé thành công!", { toastId: 'update-success' });
            ...
            // Thêm thuộc tính toastId
            toast.success("Đã tạo vé mới thành công!", { toastId: 'create-success' });
    ...
    const getStatusBadge = (status) => {
        switch ((status || '').toUpperCase()) {
            case 'PENDING':
            case 'UNPAID': return <span className="badge bg-warning text-dark">⏳ Chờ TT</span>;
    ...
    // --- HÀM TÍNH TOÁN LẠI GIÁ VÉ ---
    const calculateDisplayPrice = (ticket, allTicketsInFlight) => {
        const isInfant = ticket.isInfant || ticket.passengerType === 'INFANT' || ticket.passengerDob === '2025-09-01';
        if (isInfant) {
            const adultTicket = allTicketsInFlight.find(t =>
                !t.isInfant && t.passengerType !== 'INFANT' && t.passengerDob !== '2025-09-01'
            );
            if (adultTicket) return adultTicket.price * 0.1;
        }
        return ticket.price;
    };
    ...
    const calculateCorrectTotal = (booking) => {
        ...
        flightIds.forEach(fid => {
            const ticketsInFlight = booking.tickets.filter(t => t.flight?.id === fid);
            ticketsInFlight.forEach(t => {
                total += calculateDisplayPrice(t, ticketsInFlight);
            });
        });
        return total;
    };
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.5fr 1.5fr', gap: '15px', paddingBottom: '15px', borderBottom: '1px solid #eee', marginBottom: '15px' }}>
                    ...
                    <div>
                        ...
                        <div className="text-dark fw-bold" style={{ fontSize: '1.1em' }}>
                            {new Date(flight?.departureTime).toLocaleString('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })}
                        </div>
                    </div>
                    <div>
                        ...
                        <div className="text-dark fw-bold" style={{ fontSize: '1.1em' }}>
                            {new Date(flight?.arrivalTime).toLocaleString('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })}
                        </div>
                    </div>
                </div>
    ...
    const invoiceTotal = selectedBooking ? calculateCorrectTotal(selectedBooking) : 0;
        ...
        <div className="container-fluid" style={{ fontFamily: 'Arial, sans-serif' }}>
            ...
            {confirmModal.show && confirmModal.booking && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1050, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div className="card shadow-lg" style={{ width: '600px', maxWidth: '90%', border: 'none', borderRadius: '10px', overflow: 'hidden' }}>
                        <div className={`card-header text-white ${confirmModal.actionType === 'DELETE' ? 'bg-danger' : (confirmModal.actionType === 'PAID' ? 'bg-success' : 'bg-secondary')}`} style={{ padding: '15px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <div className="card-body p-4">
                            ...
                            <div style={{ backgroundColor: '#f8f9fa', padding: '15px', borderRadius: '8px', border: '1px solid #eee' }}>
                                ...
                                <div className="d-flex justify-content-between border-top pt-2 mt-2"><span className="text-muted">Tổng tiền:</span><span className="fw-bold text-danger fs-5">{formatCurrency(calculateCorrectTotal(confirmModal.booking))}</span></div>
                            </div>
                        </div>
                        <div className="card-footer bg-white d-flex justify-content-end gap-2 p-3">
                            ...
                            <button className={`btn ${confirmModal.actionType === 'DELETE' ? 'btn-danger' : (confirmModal.actionType === 'PAID' ? 'btn-success' : 'btn-primary')}`} onClick={confirmAction}>{confirmModal.actionType === 'DELETE' ? 'Xác nhận XÓA' : 'Xác nhận'}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {showInvoice && selectedBooking && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', backgroundColor: 'rgba(0,0,0,0.6)', zIndex: 9999, display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                    <div style={{ backgroundColor: 'white', borderRadius: '8px', overflow: 'hidden', maxWidth: '850px', width: '95%', maxHeight: '95vh', display: 'flex', flexDirection: 'column' }}>
                        <div style={{ overflowY: 'auto', flex: 1 }}>
                            <div id="invoice-content" style={{ padding: '30px', backgroundColor: 'white', color: '#333', fontFamily: 'Arial, sans-serif' }}>
                                ...
                                <div style={{ borderTop: '3px solid #eee', paddingTop: '20px', marginTop: '10px', textAlign: 'right' }}>
                                    ...
                                    <strong style={{ fontSize: '32px', color: '#d9534f', fontWeight: '800' }}>{formatCurrency(invoiceTotal)}</strong>
                                </div>
